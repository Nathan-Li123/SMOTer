...
def track_and_eval_bensmot(out_dir, data, preds, dataset_name):
    ...
    del preds
    split = 'BenSMOT-val'
    bensmot_out_dir = out_dir + '/bensmoteval/{}/pred/data/'.format(split)
    save_cocojson_as_mottxt(
        bensmot_out_dir, videos, video2images, per_image_preds)
    eval_track(out_dir + '/bensmoteval')


def eval_track(out_dir):
    freeze_support()
    ...
    config['GT_FOLDER'] = 'datasets/bensmot'
    config['SPLIT_TO_EVAL'] = 'val'
    config['TRACKERS_FOLDER'] = out_dir
    config['BENCHMARK'] = 'BenSMOT'
...
def save_cocojson_as_mottxt(out_dir, videos, video2images, per_image_preds):
    ...
    for video in videos:
        ...
        cate_name = file_name.split('/')[0]
        cate_dir = out_dir + '/' + cate_name
...
class BenSMOTEvaluator(COCOEvaluator):
