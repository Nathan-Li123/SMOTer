...
def track_and_eval_cvid(out_dir, data, preds, dataset_name):
    ...
    del preds
    split = 'CVID-val'
    cvid_out_dir = out_dir + '/cvideval/{}/pred/data/'.format(split)
    save_cocojson_as_mottxt(
        cvid_out_dir, videos, video2images, per_image_preds)
    eval_track(out_dir + '/cvideval')
...
class CVIDEvaluator(COCOEvaluator):
